...
export class NpmExplorerService {
  ...
  getNPMRegistryState(): Observable<any> {
    return this.http.get<any>('http://localhost:3000/packageInfo').pipe(
      map(response => {
        const packageInfo = {
          uuid: response.uuid,
        ...
        const valueOfStorage: any = { url: `http://localhost:3000/packageInfo`, res: JSON.stringify(response) };
  ...
  getNpmLibraryInfo(libraryName: string): Observable<any> {
    return this.http.get<any>(`http://localhost:3000/packageInfo/${libraryName}`).pipe(
      map(response => {
        ...
        const valueOfStorage: any = { url: `http://localhost:3000/packageInfo/${libraryName}`, res: JSON.stringify(response) };
  ...
  getNpmSearchText(text: string): Observable<any> {
    return this.http.get<any>(`http://localhost:3000/packageInfo/search/${text}`).pipe(
      map(response => {

        const valueOfStorage: any = { url: `http://localhost:3000/packageInfo/search/${text}`, res: JSON.stringify(response)};
  ...
  async addNpmRegistry(request: any): Promise<any> {
    try {
      const response = await this.http.post('http://localhost:3000/npm', request).toPromise();
      return response;
    } catch (error) {
      console.log(error);
      throw error;
  ...
  async getResultById(id: number): Promise<any[]> {
    try {
      const response: any = await this.http.get(`http://localhost:3000/npm/${id}`).toPromise();
